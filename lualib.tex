...
\begin{luacode*}
	...
	function tikzArrow_relative(start_p, end_p, style, label)
	-- default parameters
	  ...
	  label = label or ''
	-- draw the arrow
	  local arrow = string.format("\\path[%s] (%s) edge node(testarrow) {%s} (%s);", style, start_p, label, end_p)
	  tex.print(arrow)
	end
	...
	function tikzCoredBranch(origin_x, origin_y, radius, angle, core_name, nb_branch, branch_length, branch_name, label_distance, start_dot, end_dot, tucker)
		...
		--draw the branches
		for i=1,nb_branch do
			local string = string.format("\\draw[] (%f+%f ,%f ) arc (0:%f:%fcm) node[] ("..branch_name[i]..") {};", 
			origin_x, radius, origin_y, (i-1)*angle_seperation, radius)
			local branch = string.format("\\draw[] ("..branch_name[i]..".center) -- ++(%f:%f) ++(%f:%f) node["..style_factor.."] (dBranch"..i..") {%s};",
			(i-1)*angle_seperation, branch_length, (i-1)*angle_seperation, label_distance,branch_name[i])
			tex.print(string..branch)
			local string_dbranch = string.format("\\node (res"..i..") at ($("..branch_name[i]..")!0.5!(dBranch"..i..")$) {};")
			tex.print(string_dbranch)
			if tucker then
			  local branch = string.format("\\draw[] (dBranch"..i..") -- ++(%f:%f) ++(%f:%f) node[] (end"..i..") {n"..i.."};",
